...
class Certificados{
	...
	function validarCertificado($nombreCerPem = null){
		...
		if (file_exists($nombreCerPem)){
			...
			$salida = shell_exec('openssl x509 -in '.$nombreCerPem.' -noout -subject ');
			// $salida = str_replace('notBefore=', '', $salida);
			$razon_social = array();
			//$rfc = array();
			// preg_match('#/OU=(.*)#',			$salida,$info_preg);
			preg_match('#/CN=(.*)/name#',	$salida,$razon_social);
			//preg_match('#/x500UniqueIdentifier=(.*)/serialNumber#',	$salida,$rfc);
			$rfc =trim(substr($salida, (strpos($salida, "Identifier=") + 11), 13));
			//localiza Identifier=
			
			if(!empty($razon_social)){
				$this->_estableceError(1, null, array('Matriz' => $razon_social[1], "salida"=> $salida, "razon_social"=> $razon_social[1], "rfc"=>$rfc));
				return $this->_return;
			}else {
